...
const Login = () => {
  ...
  const loginSchema = z.object({
    ...
    password: z.string().min(6, { message: t('login.error.password') }),
  });
      ...
      <div className="relative z-10 w-full max-w-[450px] mx-4">
        <div className="bg-[rgba(15,10,28,0.55)] backdrop-blur-xl border border-endor-border-hl/30 rounded-3xl p-10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
          <div className="text-center mb-10">
            <h1 className="text-4xl font-bold mb-2 tracking-tight text-white">{t('login.title')}</h1>
            <p className="text-endor-muted font-medium">{t('login.subtitle')}</p>
          </div>

          <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
            <div className="space-y-2">
              <label className="text-sm font-semibold text-endor-muted ml-1">{t('login.userLabel')}</label>
              <input
                {...register('email')}
                type="text"
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-endor-muted ml-1">{t('login.passLabel')}</label>
              <input
                {...register('password')}
                type="password"
                placeholder="........"
